...
// Accepts serial input from pong2p.py to control both human players via MindWave EEG headset
// In this version, input from pong2p.py as attention values is used as absolute bat position. Up = increased attention
...
int inBytea = 0;               // for serial input
int inByteb = 0;               // for serial input
...
// check the second ship (right side)
  ...
  */
    // Player 1 (Human connected via EEG headset)
    while (Serial.available() > 1) {
      inBytea = Serial.read();
      inByteb = Serial.read();
      Serial.print((inBytea - 48));
    }
    if (sh2y>y && (inBytea - 48) > 4) {
      sh2y--;
    }
    if (sh2y<y && (inBytea - 48) > 4) {
      sh2y++;
    ...
    // Player 2 (Human connected via EEG headset)
      Serial.print((inByteb - 48));
      Serial.print(" ");
    if (sh2y>y && (inByteb - 48) > 4) {
      sh2y--;
    }
    if (sh2y<y && (inByteb - 48) > 4) {
      sh2y++;
